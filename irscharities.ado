...
* Program used to access, parse, and clean form 990 data on 501(c)(3)          *
...
prog def irscharities
	...
	// Store the location of the dictionary filepath
	loc dict `"`r(fn)'"'
	...
	// Set locals for area names
	loc areas area1 area2 area3 area4 area5
	
	// Set locals for region names
	loc regions region1 region2 region3
	
	// Entire US 
	loc all `region1' `region2' `region3'
	
	// Set local with all valid arguments
	loc validargs `region1' `region2' `region3' `areas' `regions' all
	
	// Test if the user passed a valid argument
	if `: list state in validargs' == 0 {
	
		// If an invalid argument was passed
		di as err "Not a valid argument for the State parameter.  Specify "  ///   
		`"one of `validargs'"'
	...
	// If an area argument is passed expand it into the constituent states
	else if `: list state in areas' == 1  loc state ``state''
	
	// If a region argument is passed expand it into the constituent states
	else if `: list state in regions' == 1 loc state ``state''
	
	// Local containing the base for the URL where the files will be retrieved
	loc urlbase `"https://www.irs.gov/pub/irs-soi/eo_"'
	
	// Loop over the arguments
	forv i = 1/`: word count `state'' {
	
		// Makes the state arguments capitalized
		loc thisstate "`: word `i' of `state''"
		
		// Constructs the URL where the data are located
		loc url `"`urlbase'`thisstate'.zip"'
	
		// Create Tempfile for Data
		tempfile zipdat`thisstate'
		
		// Drop if tempfile exists
		cap rm "`zipdat`thisstate''.zip"
		
		// Copy the ASCII Text file, which is zipped by default
		qui: copy "`url'" "`zipdat`thisstate''.zip"
		
		// Record the download time
		loc dltime `"`dltime'"' `""`c(current_date)' `c(current_time)'""'
		
		// Record the download url
		loc dlurl `dlurl' "`url'"
		
		// Extract the files
		qui: unzipfile "`zipdat`thisstate''.zip", replace
		
	} // End of Downloads
	
	// Print status message for user
	di as res "Finished downloading files.  Parsing and cleaning now"
	...
	preserve
		...
		forv i = 1/`: word count `state'' {
			...
			// Saves the data to the file specified by the user on first iteration
			if `i' == 1 save `"`save'.dta"', `replace'
			
			// On subsequent iterations
			else {
				
				// Append the previously read/parsed data to these data
				append using `"`save'.dta"'
				
				// Save over the file specified by the user (this should only 
				// rewrite the file created by the user on this call)
				save `"`save'.dta"', replace
				
			} // End ELSE Block for other iterations
		...
		forv i = 1/`: word count `state'' {
			...
			// Add download site as characteristic of the data set
		...
		// Makes income amount a negative value if negative symbol placed in 
		// the indicator column
		qui: replace incomeamt = incomeamt * -1 if negincome=="-"
		
		// Do the same for the revenue variable
		qui: replace revenueamt = revenueamt * -1 if negrevenue=="-"
		
		// Drop variable
		qui: drop na
				
		// Define value labels for affiliation code
		{
		...
		// Define value labels for deduction code				
		{
		...
		// Define value labels for foundation code				
		{
		la def foundcd														 ///   
		...
		9 "Suspense" 10 "Church 170(b)(1)(A)(i)"							 ///   
		...
		21 "509(a)(3) Type I" 22 "509(a)(3) Type II"						 ///   
		23 "509(a)(3) Type III - Functionally Integrated"					 ///   
		24 "509(a)(3) Type III - Not Functionally Integrated", modify 
		}
		
		// Defines value labels for organization types	
		{
		la def orgtype	1 "Corporation" 2 "Trust" 3 "Cooperative"			 ///   
						4 "Partnership" 5 "Association", modify 
		}
		
		// Defines value labels for status types
		{
		la def status														 ///   
		1 "Unconditional Exemption" 2 "Conditional Exemption"				 ///   
		12 "Trust Described in section 4947(a)(2) of the IR code"			 ///   
		25 "Organization Terminating its Private Foundation Status"			 ///   
		32 "Organization that Did Not Respond to an IRS CP140 notice", modify 
		}
		
		// Defines value labels for assets variable
		{
		la def incass	0 "$0" 1 "$1 to $9,999" 2 "$10,000 to $24,999"		 ///   
		3 "$25,000 to $99,999" 4 "$100,000 to $499,999"						 ///   
		5 "$500,000 to $999,999" 6 "$1,000,000 to $4,999,999"				 ///   
		7 "$5,000,000 to $9,999,999" 8 "$10,000,000 to $49,999,999"			 ///   
		9 "$50,000,000 +", modify 
		}
		
		// Defines value labels for filing requirements variable
		{
		la def filereq														 ///   
		...
		14 "990 - Not Required to File (State/Political Subdivisions", modify
		}
		
		// Defines value labels for 990-PF Form requirement		
		{
		la def pfreq	1 "Required a 990-PF Return"						 ///   
						0 "Did Not Require a 990-PF Return", modify 
		}
		
		// The next block defines codes used for mapping activity codes 
		// Religious Activities
		{
		la def activities 													 ///   
		0	"No Information Available" 1 	"Church, synagogue, etc"		 ///   
		2	"Association or convention of churches"							 ///   
		3	"Religious order" 4	"Church auxiliary" 5	"Mission"			 ///   
		6	"Missionary activities" 7	"Evangelism"						 ///   
		...
		32	"Nursery school" 33	"Faculty group"								 ///   
		...
		40	"Scholarships (other)" 41	"Student loans"						 ///   
		42	"Student housing activities" 43	"Other student aid"				 ///   
		44	"Student exchange with foreign country"							 ///   
		45	"Student operated business" 46	"Private school"				 ///   
		...
		{
		la def activities 													 ///   
		60	"Museum, zoo, planetarium, etc." 61	"Library"					 ///   
		62	"Historical site, records or reenactment" 63 "Monument"			 ///   
		64	"Commemorative event (centennial, festival, pageant, etc.)"		 ///   
		65	"Fair" 88	"Community theatrical group"						 ///   
		89	"Singing society or group" 90	"Cultural performances"			 ///   
		91	"Art exhibit" 92	"Literary activities"						 ///   
		...
		{
		la def activities 													 ///   
		120	"Publishing activities" 121	"Radio or television broadcasting"	 ///   
		122	"Producing films"												 ///   
		...
		126	"Apprentice training" 149	"Other instruction and training", modify 
		}
		
		// Health Services and Related Activities
		{
		la def activities 													 ///   
		150	"Hospital" 151 "Hospital auxiliary"								 ///   
		152	"Nursing or convalescent home"									 ///   
		153	"Care and housing for the aged (see also 382)"					 ///   
		154	"Health clinic" 155	"Rural medical facility" 156 "Blood bank"	 ///   
		157	"Cooperative hospital service organization"						 ///   
		158	"Rescue and emergency service" 159 "Nurses register or bureau"	 ///   
		160	"Aid to the handicapped (see also 031)"							 ///   
		161	"Scientific research (diseases)" 162 "Other medical research"	 ///   
		163	"Health insurance (medical, dental, optical, etc.)"				 ///   
		164	"Prepared group health plan" 165 "Community health planning"	 ///   
		166	"Mental health care" 167 "Group medical practice association"	 ///   
		...
		{
		la def activities 													 ///   
		200	"Business promotion" 201 "Real estate association"				 ///   
		202	"Board of trade" 203 "Regulating business"						 ///   
		204	"Promotion of fair business practices"							 ///   
		205	"Professional association"										 ///   
		206	"Professional association auxiliary"							 ///   
		207	"Industry trade shows" 208 "Convention displays"				 ///   
		...
		213	"Tourist bureau" 229 "Other business or professional group", modify 
		}
		
		// Farming and Related Activities
		{
		la def activities 													 ///   
		230	"Farming" 231 "Farm bureau" 232 "Agricultural group"			 ///   
		233	"Horticultural group"											 ///   
		234	"Farmers cooperative marketing or purchasing"					 ///   
		235	"Farmers cooperative marketing or purchasing"					 ///   
		236	"Dairy herd improvement association" 237 "Breeders association"  ///   
		...
		251	"Credit union" 252	"Reserve funds or insurance"				 ///   
		...
		266	"Strike benefits" 267 "Unemployment benefits"					 ///   
		268	"Pension or retirement benefits" 269 "Vacation benefits"		 ///   
		...
		{
		la def activities 													 ///   
		280	"Country club" 281 "Hobby club" 282 "Dinner club" 				 ///   
		283 "Variety club" 284 "Dog club" 285 "Women's club"				 ///   
		286	"Hunting or fishing club" 287 "Swimming or tennis club"			 ///   
		288	"Other sports club" 296	"Community center"						 ///   
		297	"Community recreational facilities (park, playground, etc)"		 ///   
		298	"Training in sports" 299 "Travel tours"							 ///   
		...
		321	"Boys Club, Little League, etc." 322 "FFA, FHA, 4-H club, etc."  ///   
		323	"Key club" 324 "YMCA, YWCA, YMCA, etc." 325	"Camp"				 ///   
		...
		355	"Wildlife sanctuary or refuge" 356 "Garden club"				 ///   
		379	"Other conservation, environmental or beautification activities", modify 
		}
		
		// Housing Activities
		{
		la def activities 380 "Low-income housing"							 ///   
		...
		462	"Legal aid to indigents" 463 "Providing bail" 					 ///   
		...
		{
		la def activities 510 "Firearms control"							 ///   
		511	"Selective Service System" 512 "National defense policy"		 ///   
		513	"Weapons systems" 514 "Government spending"						 ///   
		515	"Taxes or tax exemption" 516 "Separation of church and state"	 ///   
		517	"Government aid to parochial schools"							 ///   
		518	"U.S. foreign policy" 519 "U.S. military involvement"			 ///   
		520	"Pacifism and peace" 521 "Economic-political system of U.S."	 ///   
		522	"Anti-communism" 523 "Right to work" 524 "Zoning or rezoning" 	 ///   
		525 "Location of highway or transportation system"					 ///   
		526	"Rights of criminal defendants" 527	"Capital punishment"		 ///   
		528	"Stricter law enforcement" 529 "Ecology or conservation"		 ///   
		530	"Protection of consumer interests" 531 "Medical care service"	 ///   
		532	"Welfare systems" 533 "Urban renewal"							 ///   
		534	"Busing student to achieve racial balance"						 ///   
		535	"Racial integration" 536 "Use of intoxicating beverage"			 ///   
		537	"Use of drugs or narcotics" 538	"Use of tobacco"				 ///   
		539	"Prohibition of erotica" 540 "Sex education in public schools"	 ///   
		541	"Population control" 542 "Birth control methods"				 ///   
		543	"Legalized abortion" 559 "Other matters", modify 
		}
		...
		562	"Other loans to individuals" 563 "Marriage counseling"			 ///   
		564	"Family planning" 565 "Credit counseling an assistance"			 ///   
		566	"Job training, counseling, or assistance"						 ///   
		567	"Draft counseling" 568 "Vocational counseling"					 ///   
		...
		574	"Day care center" 575 "Services for the aged (see also 153 ad 382)", modify 
		}
		
		// Activities Purposes and Activities
		{
		la def activities 													 ///   
		600 "Community Chest, United Way, etc." 601 "Booster club"			 ///    
		...
		{
		la def activities 900 "Cemetery or burial activities"				 ///   
		...
		906	"Consumer interest group" 907 "Veterans activities"				 ///   
		...
		911	"Foreign organization" 912 "Title holding corporation"			 ///   
		...
		918	"Book, gift  or supply store" 919 "Advertising"					 ///   
		920	"Association of employees" 921 "Loans or credit reporting"		 ///   
		...
		927 "Fundraising" 928 "4947(a)(2) trust"							 ///   
		...
		995 "Described in section 509(a)(2) of the Code" 998 "Unspecified", modify 
		}
		
		// Defines NTEE Class types
		{
		la def nteeclass 1 "Arts, Culture and Humanities"					 ///   
		2 "Educational Institutions and Related Activities"					 ///   
		3 "Environmental Quality, Protection and Beautification"			 ///   
		4 "Animal-Related" 5 "Health - General and Rehabilitative"			 ///   
		6 "Mental Health, Crisis Intervention"								 ///   
		7 "Diseases, Disorders, Medical Disciplines" 8 "Medical Research"    ///   
		9 "Crime, Legal-Related" 10 "Employment, Job-Related"				 ///   
		11 "Food, Agriculture and Nutrition" 12 "Housing, Shelter"			 ///   
		13 "Public Safety, Disaster Preparedness and Relief"				 ///   
		14 "Recreation, Sports, Leisure, Athletics" 15 "Youth Development"	 ///   
		...
		26 "Unknown" .n "No Information Provided", modify 
		}
		
		/* The next block is used to define value labels for NTEE classification 
		codes.  There are a lot of these so the section is fairly long.  Curly 
		braces are added so these chunks can be folded. */
		
// 1 - Arts, Culture and Humanities
		{
		la def nteecodes 101 "Alliance/Advocacy Organizations" 				 ///   
		...
		123 "Cultural, Ethnic Awareness" 125 "Arts Education" 				 ///   
		126 "Arts Council/Agency" 130 "Media, Communications Organizations"  ///   
		131 "Film, Video" 132 "Television" 133 "Printing, Publishing" 		 ///   
		134 "Radio" 140 "Visual Arts Organizations" 						 ///   
		150 "Museum, Museum Activities" 151 "Art Museums" 					 ///   
		152 "Children's Museums" 154 "History Museums" 						 ///   
		156 "Natural History, Natural Science Museums" 						 ///   
		157 "Science and Technology Museums" 								 ///   
		160 "Performing Arts Organizations" 161 "Performing Arts Centers"    ///   
		162 "Dance" 163 "Ballet" 165 "Theater" 168 "Music" 					 ///   
		169 "Symphony Orchestras" 171 "Opera" 172 "Singing, Choral" 		 ///   
		173 "Music Groups, Bands, Ensembles" 174 "Performing Arts Schools"   ///   
		...
		// 2 - Education
		{
		la def nteecodes 201 "Alliance/Advocacy Organizations" 				 ///   
		...
		224 "Primary, Elementary Schools" 225 "Secondary, High School" 		 ///   
		...
		260 "Adult, Continuing Education" 270 "Libraries" 					 ///   
		280 "Student Services, Organizations of Students" 					 ///   
		282 "Scholarships, Student Financial Aid Services, Awards" 			 ///   
		283 "Student Sororities, Fraternities" 284 "Alumni Associations" 	 ///   
		290 "Educational Services and Schools - Other" 						 ///   
		292 "Remedial Reading, Reading Encouragement" 						 ///   
		294 "Parent/Teacher Group" 299 "Education N.E.C.", modify 
		} 

		// 3 - Environmental Quality, Protection and Beautification 
		{
		la def nteecodes 301 "Alliance/Advocacy Organizations" 				 ///   
		...
		// 4 - Animal-Related 
		{
		la def nteecodes 401 "Alliance/Advocacy Organizations" 				 ///   
		...
		419 "Nonmonetary Support N.E.C." 420 "Animal Protection and Welfare" ///   
		430 "Wildlife Preservation, Protection" 							 ///   
		431 "Protection of Endangered Species" 								 ///   
		432 "Bird Sanctuary, Preserve" 433 "Fisheries Resources" 			 ///   
		434 "Wildlife Sanctuary, Refuge" 440 "Veterinary Services" 			 ///   
		450 "Zoo, Zoological Society" 										 ///   
		460 "Other Services - Specialty Animals" 							 ///   
		461 "Animal Training, Behavior" 499 "Animal-Related N.E.C.", modify 
		}

		// 5 - Health - General and Rehabilitative
		{
		la def nteecodes 501 "Alliance/Advocacy Organizations"				 ///   
		...
		521 "Community Health Systems" 522 "Hospital, General"				 ///   
		...
		542 "Family Planning Centers" 550 "Rehabilitative Medical Services"	 ///   
		560 "Health Support Services" 561 "Blood Supply Related"			 ///   
		...
		592 "Home Health Care" 599 "Health - General and Rehabilitative N.E.C.", modify 
		} 

		// 6 - Mental Health, Crisis Intervention
		{
		la def nteecodes 601 "Alliance/Advocacy Organizations"				 ///   
		...
		642 "Rape Victim Services" 650 "Addictive Disorders N.E.C."			 ///   
		652 "Smoking Addiction" 653 "Eating Disorder, Addiction"			 ///   
		654 "Gambling Addiction" 660 "Counseling, Support Groups"			 ///   
		...
		{
		la def nteecodes 701 "Alliance/Advocacy Organizations"				 ///   
		...
		725 "Down Syndrome" 730 "Cancer" 740 "Diseases of Specific Organs"	 ///   
		...
		744 "Kidney Disease" 745 "Lung Disease" 748 "Brain Disorders"		 ///   
		750 "Nerve, Muscle and Bone Diseases" 751 "Arthritis"				 ///   
		754 "Epilepsy" 760 "Allergy Related Diseases" 761 "Asthma"			 ///   
		770 "Digestive Diseases, Disorders"									 ///   
		780 "Specifically Named Diseases" 781 "AIDS"						 ///   
		783 "Alzheimer's Disease" 784 "Autism" 790 "Medical Disciplines"	 ///   
		792 "Biomedicine, Bioengineering" 794 "Geriatrics"					 ///   
		796 "Neurology, Neuroscience" 798 "Pediatrics" 797 "Surgery"		 ///   
		799 "Diseases, Disorders, Medical Disciplines N.E.C.", modify  
		}

		// 8 - Medical Research
		{
		la def nteecodes 801 "Alliance/Advocacy Organizations"				 ///   
		...
		825 "Down Syndrome Research" 830 "Cancer Research"					 ///   
		840 "Specific Organ Research" 841 "Eye Research"					 ///   
		842 "Ear and Throat Research" 843 "Heart, Circulatory Research"		 ///   
		844 "Kidney Research" 845 "Lung Research"							 ///   
		848 "Brain Disorders Research" 850 "Nerve, Muscle, Bone Research"	 ///   
		851 "Arthritis Research" 854 "Epilepsy Research"					 ///   
		860 "Allergy Related Disease Research" 861 "Asthma Research"		 ///   
		870 "Digestive Disease, Disorder Research"							 ///   
		880 "Specifically Named Diseases Research" 881 "AIDS Research"		 ///   
		883 "Alzheimer's Disease Research" 884 "Autism Research"			 ///   
		890 "Medical Specialty Research"									 ///   
		892 "Biomedicine, Bioengineering Research"							 ///   
		894 "Geriatrics Research" 896 "Neurology, Neuroscience Research"	 ///   
		898 "Pediatrics Research" 897 "Surgery Research"					 ///   
		899 "Medical Research N.E.C.", modify 
		} 

		// 9 - Crime, Legal-Related
		{
		la def nteecodes 901 "Alliance/Advocacy Organizations"				 ///   
		...
		919 "Nonmonetary Support N.E.C." 920 "Crime Prevention N.E.C."		 ///   
		921 "Delinquency Prevention" 923 "Drunk Driving Related"			 ///   
		...
		944 "Prison Alternatives" 950 "Administration of Justice, Courts"	 ///   
		...
		971 "Spouse Abuse, Prevention of" 972 "Child Abuse, Prevention of"	 ///   
		973 "Sexual Abuse, Prevention of" 980 "Legal Services"				 ///   
		...
		{
		la def nteecodes 1001 "Alliance/Advocacy Organizations"				 ///   
		...
		1022 "Vocational Training" 1030 "Vocational Rehabilitation"			 ///   
		...
		{
		la def nteecodes 1101 "Alliance/Advocacy Organizations"				 ///   
		...
		1119 "Nonmonetary Support N.E.C." 1120 "Agricultural Programs"		 ///   
		...
		1131 "Food Banks, Food Pantries" 1134 "Congregate Meals"			 ///   
		1135 "Eatery, Agency, Organization Sponsored"						 ///   
		1136 "Meals on Wheels" 1140 "Nutrition Programs"					 ///   
		1150 "Home Economics" 1199 "Food, Agriculture, and Nutrition N.E.C.", modify  
		}

		// 12 - Housing, Shelter
		{
		la def nteecodes 1201 "Alliance/Advocacy Organizations"				 ///   
		...
		1225 "Housing Rehabilitation" 1230 "Housing Search Assistance"		 ///   
		...
		// 13 - Public Safety, Disaster Preparedness and Relief
		{
		la def nteecodes 1301 "Alliance/Advocacy Organizations" 			 ///   
		...
		1340 "Safety Education" 1341 "First Aid Training, Services"			 ///   
		...
		{
		la def nteecodes 1401 "Alliance/Advocacy Organizations"				 ///   
		...
		1431 "Community Recreational Centers" 1432 "Parks and Playgrounds"	 ///   
		...
		1461 "Fishing, Hunting Clubs" 1462 "Basketball"						 ///   
		1463 "Baseball, Softball" 1464 "Soccer Clubs, Leagues"				 ///   
		1465 "Football Clubs, Leagues"										 ///   
		1466 "Tennis, Racquet Sports Clubs, Leagues"						 ///   
		1467 "Swimming, Water Recreation"									 ///   
		1468 "Winter Sports (Snow and Ice)" 1469 "Equestrian, Riding"		 ///   
		1459 "Golf"	1470 "Amateur Sports Competitions"						 ///   
		1471 "Olympics Committees and Related International Competitions"	 ///   
		1472 "Special Olympics" 1480 "Professional Athletic Leagues"		 ///   
		1499 "Recreation, Sports, Leisure, Athletics N.E.C.", modify 
		}

		// 15 - Youth Development 
		{
		la def nteecodes 1501 "Alliance/Advocacy Organizations"				 ///   
		...
		1520 "Youth Centers, Clubs, Multipurpose" 1521 "Boys Clubs"			 ///   
		1522 "Girls Clubs" 1523 "Boys and Girls Clubs (Combined)"			 ///   
		1530 "Adult, Child Matching Programs"								 ///   
		1531 "Big Brothers, Big Sisters" 1540 "Scouting Organizations"		 ///   
		1541 "Boy Scouts of America" 1542 "Girl Scouts of the U.S.A."		 ///   
		1543 "Camp Fire" 1550 "Youth Development Programs, Other"			 ///   
		...
		// 16 - Human Services - Multipurpose and Other 
		{
		la def nteecodes 1601 "Alliance/Advocacy Organizations"				 ///   
		...
		1621 "American Red Cross" 1622 "Urban League" 1624 "Salvation Army"	 ///   
		...
		1629 "Thrift Shops"	1630 "Children's, Youth Services"				 ///   
		1631 "Adoption" 1632 "Foster Care" 1633 "Child Day Care"			 ///   
		1640 "Family Services" 1642 "Single Parent Agencies, Services"		 ///   
		1643 "Family Violence Shelters, Services"							 ///   
		1644 "Homemaker, Home Health Aide"									 ///   
		1645 "Family Services, Adolescent Parents"							 ///   
		1646 "Family Counseling" 1650 "Personal Social Services"			 ///   
		1651 "Financial Counseling, Money Management"						 ///   
		1652 "Transportation, Free or Subsidized" 1658 "Gift Distribution"	 ///   
		1660 "Emergency Assistance (Food, Clothing, Cash)"					 ///   
		1661 "Travelers' Aid" 1662 "Victims' Services"						 ///   
		1670 "Residential, Custodial Care"									 ///   
		1672 "Half-Way House (Short-Term Residential Care)"					 ///   
		1673 "Group Home (Long Term)" 1674 "Hospice"						 ///   
		...
		// 17 - International, Foreign Affairs and National Security 
		{
		la def nteecodes 1701 "Alliance/Advocacy Organizations"				 ///   
		...
		1733 "International Relief" 1740 "International Peace and Security"	 ///   
		1741 "Arms Control, Peace Organizations"							 ///   
		1742 "United Nations Association" 1743 "National Security, Domestic" ///   
		...
		// 18 - Civil Rights, Social Action, Advocacy 
		{
		la def nteecodes 1801 "Alliance/Advocacy Organizations"				 ///   
		...
		1822 "Minority Rights" 1823 "Disabled Persons' Rights"				 ///   
		1824 "Women's Rights" 1825 "Seniors' Rights"						 ///   
		1826 "Lesbian, Gay Rights" 1830 "Intergroup, Race Relations"		 ///   
		1840 "Voter Education, Registration" 1860 "Civil Liberties Advocacy" ///   
		1861 "Reproductive Rights" 1862 "Right to Life"						 ///   
		...
		// 19 - Community Improvement, Capacity Building 
		{
		la def nteecodes 1901 "Alliance/Advocacy Organizations"				 ///   
		...
		1921 "Community Coalitions" 1922 "Neighborhood, Block Associations"	 ///   
		1930 "Economic Development"											 ///   
		1931 "Urban, Community Economic Development"						 ///   
		1932 "Rural Development" 1940 "Business and Industry"				 ///   
		1941 "Promotion of Business"										 ///   
		1943 "Management Services for Small Business, Entrepreneurs"		 ///   
		1946 "Boards of Trade" 1947 "Real Estate Organizations"				 ///   
		1950 "Nonprofit Management" 1980 "Community Service Clubs"			 ///   
		1981 "Women's Service Clubs" 1982 "Men's Service Clubs"				 ///   
		1999 "Community Improvement, Capacity Building N.E.C.", modify 
		}

		// 20 - Philanthropy, Voluntarism and Grantmaking Foundations 
		{
		la def nteecodes 2001 "Alliance/Advocacy Organizations"				 ///   
		...
		2021 "Corporate Foundations" 2022 "Private Independent Foundations"	 ///   
		2023 "Private Operating Foundations" 2030 "Public Foundations"		 ///   
		2031 "Community Foundations" 2040 "Voluntarism Promotion"			 ///   
		...
		// 21 - Science and Technology Research Institutes, Services 
		{
		la def nteecodes 2101 "Alliance/Advocacy Organizations"				 ///   
		...
		2119 "Nonmonetary Support N.E.C." 2120 "Science, General"			 ///   
		2121 "Marine Science and Oceanography"								 ///   
		2130 "Physical Sciences, Earth Sciences Research and Promotion"		 ///   
		2131 "Astronomy" 2133 "Chemistry, Chemical Engineering"				 ///   
		2134 "Mathematics" 2136 "Geology"									 ///   
		2140 "Engineering and Technology Research, Services"				 ///   
		2141 "Computer Science" 2142 "Engineering"							 ///   
		...
		// 22 - Social Science Research Institutes, Services
		{
		la def nteecodes 2201 "Alliance/Advocacy Organizations"				 ///   
		...
		2222 "Economics (as a social science)" 2223 "Behavioral Science"	 ///   
		2224 "Political Science" 2225 "Population Studies"					 ///   
		2226 "Law, International Law, Jurisprudence"						 ///   
		2230 "Interdisciplinary Research" 2231 "Black Studies"				 ///   
		2232 "Women's Studies" 2233 "Ethnic Studies" 2234 "Urban Studies"	 ///   
		2235 "International Studies"										 ///   
		2236 "Gerontology (as a social science)" 2237 "Labor Studies"		 ///   
		2299 "Social Science Research Institutes, Services N.E.C.", modify 
		}

		// 23 - Public, Society Benefit - Multipurpose and Other
		{
		la def nteecodes 2301 "Alliance/Advocacy Organizations"				 ///   
		...
		2361 "Credit Unions" 2370 "Leadership Development"					 ///   
		2380 "Public Utilities" 2390 "Consumer Protection, Safety"			 ///   
		2399 "Public, Society Benefit - Multipurpose and Other N.E.C.", modify 
		}

		// 24 - Religion-Related, Spiritual Development 
		{
		la def nteecodes 2401 "Alliance/Advocacy Organizations"				 ///   
		...
		2419 "Nonmonetary Support N.E.C." 2420 "Christian" 2421 "Protestant" ///   
		2422 "Roman Catholic" 2430 "Jewish" 2440 "Islamic" 2450 "Buddhist"	 ///   
		2470 "Hindu" 2480 "Religious Media, Communications Organizations" 	 ///   
		2481 "Religious Film, Video" 2482 "Religious Television"			 ///   
		2483 "Religious Printing, Publishing" 2484 "Religious Radio"		 ///   
		...
		// 25 - Mutual/Membership Benefit Organizations, Other
		{
		la def nteecodes 2501 "Alliance/Advocacy Organizations"				 ///   
		...
		// 26 -	Unknown
		{
			la def nteecodes 2699	"Unknown", modify 
		}

		// Creates tempvars used to clean up the NTEE Codes if the user requests it
		tempvar nteelet nteenum
		qui: g `nteelet' = upper(substr(nteecode, 1, 1))
		qui: g `nteenum' = upper(substr(nteecode, 2, 2))	
		...
		if "`nteenfix'" != "" {
			...
			foreach v in `c(ALPHA)' {
				...
				// Substitute all letters with number zero
		...
		// Recoding NTEE Classification Codes to Numeric Based on Letter
		{
			...
			// Loop over value mappings for letters
			forv i = 1/26 {
			
				// Replace any letter with its corresponding number value
				qui: replace `nteelet' = "`i'" if inlist(`nteelet', 		 ///   
				`"`: word `i' of `c(alpha)''"', `"`: word `i' of `c(ALPHA)''"')
		...
		qui: replace nteecode2 = cond(nteecode == "A6A", "171",				 ///   
								 cond(nteecode == "A6B", "172",				 ///   
								 cond(nteecode == "A6C", "173",				 ///    
		...
		// Replace with extended missing value if missing
		qui: replace nteeclass = .n if nteeclass == .
		
		// Add value labels to recoded activity codes if option nteefix used
		if "`nteenfix'" != "" la val nteecode2 nteecodes
		
		// Apply value labels to activity codes
		foreach v of var activity1 activity2 activity3 {
			
			// Label the activity variables 
			la val `v' activities
			
		} // End Loop to attach activity codes
